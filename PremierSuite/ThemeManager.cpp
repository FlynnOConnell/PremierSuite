...
ThemeManager::ThemeManager()
{
    ...
    if (!fileExists(filepath.string())) {
        ...
        outfile << m_theme;
       // LOG("Empty theme.ini file created in {}", filepath.relative_path().string());
    }

    std::ifstream file(PREMIERSUITE_DATA_PATH / "theme.ini");
    if (file.is_open())
    {
        int themeIdx;
        file >> themeIdx;
        setTheme((Theme)(themeIdx % Theme_Count));
        LOG("Theme set from theme.ini file.");
    }
}

ThemeManager::~ThemeManager()
{
    std::ofstream file(PREMIERSUITE_DATA_PATH / "theme.ini");
    if (file.is_open())
    {
        file << m_theme;
        LOG("Theme saved to theme.ini file.");
    }
}
...
//void ThemeManager::SaveToDisk(std::string fileName)
//{
//    std::filesystem::path filepath = PREMIERSUITE_DATA_PATH / std::string("default");
//    std::filesystem::path filename = filepath / fileName;
//
//    ImGui::SaveIniSettingsToDisk(filename.string().c_str());
//}

//void ThemeManager::LoadFromDisk(std::string fileName)
//{
